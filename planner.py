...
class Base_Planner(ABC):
    ...
    def __init__(self, pi_teacher_noise = "gaussian", noise_ratio = 0.2, offline=True, soft=False, prefix=''):
        ...
        self.plans_dict = {}
        self.mediator = None
        self.pi_teacher_noise = pi_teacher_noise
    ...
    def plan(self, text, n_ask=10):
        if text in self.plans_dict.keys():
            ...
            if self.pi_teacher_noise == "gaussian":
                n = len(list(plans.values()))
                for i in range(n):
                    noise = np.random.normal(1, 1, 1)
                    probs[i] += self.noise_ratio * noise
            total = sum(probs)
...
class SimpleDoorKey_Planner(Base_Planner):
    def __init__(self, offline, soft, prefix):
        ...
        self.choices = {
                ...
                "Agent sees <key>, <door>, holds <nothing>." : ["go to <key>, pick up <key>, go to <door>, open <door>", "go to <key>, pick up <key>, open <door>", "pick up <key>, go to <door>, open <door>", "go to <key>, go to <door>, use <key>", "go to <key>, pick up <key>, explore"], 
        }
