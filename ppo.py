    ...
    def update_policy(self, buffer, upp):
        for _ in range(self.epochs):
            ...
            for batch in buffer.sample(self.batch_size):
                _, next_obs_batch, _, return_batch, _, _, _, _, _, teacher_value_batch = batch
                v_next = self.critic_network(next_obs_batch).requires_grad_(True)
                predicted_values = torch.minimum(torch.maximum(return_batch + self.gamma * v_next, torch.tensor(0, device=v_next.device)), torch.tensor(upp, device=v_next.device)).requires_grad_(True)
                loss = nn.functional.mse_loss(predicted_values.unsqueeze(1), teacher_value_batch.unsqueeze(1)).requires_grad_(True)
                loss.backward()
                self.optimizer.step()
