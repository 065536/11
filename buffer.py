...
class Buffer:
    ...
    def store(self, state, next_state, action, reward, value, log_probs, teacher_probs, teacher_values):
        """
        ...
        self.teacher_values += [teacher_values]
        self.ptr += 1
    ...
    def sample(self, batch_size=64, recurrent=False):
        ...
        observations, next_observations, actions, returns, values, log_probs, teacher_probs, teacher_values = map(torch.Tensor, self.get())
        ...
        for indices in sampler:
            if recurrent:              
                ...
                teacher_value_batch = [teacher_values[self.traj_idx[i]:self.traj_idx[i+1]] for i in indices]
                ...
                teacher_value_batch =  pad_sequence(teacher_value_batch, batch_first=False).flatten(0,1)
            else:
            ...
            yield obs_batch.to(self.device), next_obs_batch.to(self.device), action_batch.to(self.device), return_batch.to(self.device), advantage_batch.to(self.device), values_batch.to(self.device), mask.to(self.device), log_prob_batch.to(self.device), teacher_prob_batch.to(self.device), teacher_value_batch.to(self.device)